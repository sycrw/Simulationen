...
function simulation() {
  ...
  fs.appendFileSync(
    "simulations.txt",
    `Simulation with ${playersAmount} players finished at ${new Date().toLocaleDateString() +
      " " +
      new Date().toLocaleTimeString()}\n`
  );
  fs.appendFileSync(
  ...
  fs.appendFileSync(
    "simulations.txt",
    `The Simulation errored ${((wins["error"] / interations) * 100).toFixed(
      2
    )}% of the times\n`
  );
